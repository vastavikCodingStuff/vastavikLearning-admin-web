...
export default function PracticePYQPage() {
  ...
  useEffect(() => {
    api.get<PYQ[]>("/api/v1/pyqs")
      .then((r) => setPYQs(r.data ?? []))
      .catch(() => {
        setPYQs([
          { id: "p1", board: "ICSE", year: "2024", subject: "Computer Applications", question: "What is autoboxing? Explain with an example in Java.", solution: "Autoboxing is the automatic conversion of primitive types to their corresponding wrapper classes. E.g., int → Integer. Example: Integer x = 5; // autoboxing", marks: 4 },
          { id: "p2", board: "ICSE", year: "2023", subject: "Computer Applications", question: "Write a program in Java to find the factorial of a number using recursion.", solution: "int factorial(int n) { if(n==0) return 1; return n * factorial(n-1); }", marks: 5 },
          { id: "p3", board: "CBSE", year: "2024", subject: "Informatics Practices", question: "Explain the difference between INNER JOIN and LEFT JOIN in SQL.", solution: "INNER JOIN returns only matching rows from both tables. LEFT JOIN returns all rows from the left table and matching rows from the right table; non-matching right rows have NULL.", marks: 3 },
          { id: "p4", board: "CBSE", year: "2023", subject: "Computer Science", question: "Write the output of the following Python code: print([x**2 for x in range(1,6)])", solution: "[1, 4, 9, 16, 25]", marks: 2 },
        ]);
      })
        ...
        {loading ? (
          ...
          <div key={pyq.id} className="bg-white rounded-xl p-5 shadow-sm border border-slate-100">
            ...
            <div className="bg-green-50 border border-green-100 rounded-lg p-3">
              ...
              <p className="text-xs text-green-900 leading-relaxed">{pyq.solution}</p>
            </div>
          </div>
        ))}
